...
class GPUParallelTrainer:
    ...
    def __init__(
        ...
        logging.info(f"Parallel Trainer initialized with {self.num_workers} workers")
        if self.use_cuda:
            logging.info(f"Using {len(self.gpu_ids)} GPUs: {self.gpu_ids}")
        else:
        ...
        **train_kwargs,
        ...
        if self.num_workers <= 1 or len(policies_to_train) <= 1 or not self.use_cuda:
            ...
            for policy_key, (policy, env, train_data) in policies_to_train.items():
                ...
                logging.info(f"Training policy {policy_key} on {device}")
            ...
            if self.pool:
                ...
                for policy_key, async_result in async_results:
                    try:
                        ...
                        logging.info(f"Error training policy {policy_key}: {e}")
...
def _train_policy_process(
    ...
    try:
        ...
        # Train the policy
        logging.info(f"Process for {policy_key} starting training on {device}")
        result = train_function(policy, env, train_data, policy_key=policy_key)
        ...
        logging.info(
            f"Process for {policy_key} completed training but no model was saved"
        )
        return {"success": result}
    except Exception as e:
        logging.info(f"Error in training process for {policy_key}: {e}")
        raise
